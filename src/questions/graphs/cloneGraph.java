...
/**
 * <div class="_1l1MA"><p>Given a reference of a node in a <strong><a href="https://en.wikipedia.org/wiki/Connectivity_(graph_theory)#Connected_graph" target="_blank">connected</a></strong> undirected graph.</p>
 *
...
public class cloneGraph {
    Set<Node> cloned = new HashSet<>();
    public Node cloneGraph(Node node) {
        if(node == null) return node;
        Node answerNode = new Node(node.val);
        cloneG(node, answerNode);
        return answerNode;
    }

    private void cloneG(Node realNode, Node cloneNode){
        cloned.add(cloneNode);
        realNode.neighbors.forEach(neighbor->{
            Optional<Node> optionalNode = cloned.stream().filter(node -> node.val==neighbor.val).findAny();
            if(optionalNode.isPresent()){
                cloneNode.neighbors.add(optionalNode.get());
            } else {
                Node node = new Node(neighbor.val);
                cloneNode.neighbors.add(node);
                cloneG(neighbor, node);
            }
        });
    }
...
//                 so now if it is not visited, create a new node
                Node newNode = new Node(n.val);
//                 add this node as the neighbor of the prev copied node
                copy.neighbors.add(newNode);
//                 make dfs call for this unvisited node to discover whether its adjacent nodes are explored or not
